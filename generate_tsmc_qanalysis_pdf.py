#!/usr/bin/env python3
"""Generate a PDF from the TSMC Quartr-enhanced analysis markdown."""

import re
from fpdf import FPDF

INPUT = "TSMC-qanalysis.md"
OUTPUT = "docs/pdfs/TSMC-qanalysis.pdf"
...
class MarkdownPDF(FPDF):
    ...
    def table_row(self, cells, bold=False, col_widths=None):
        ...
        for j, cell_text in enumerate(cells):
            ...
            text = self._sanitize(cell_text)
            max_chars = int(w / 2)
            if len(text) > max_chars:
                text = text[:max_chars - 3] + "..."
            border = 1 if bold else "LBR"
            self.cell(w, h, text, border=border, fill=True)
        self.ln(h)
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "TSMC (TSM) - Quartr-Enhanced Investment Analysis", align="C")
            self.set_text_color(0, 0, 0)
            self.ln(12)
...
def parse_and_render(md_path, output_path):
    ...
    # Skip title and metadata lines (up to first ##)
    while i < len(lines):
        line = lines[i].rstrip()
        if line.startswith("## "):
            break
    ...
    while i < len(lines):
        ...
        if line.startswith("|"):
            ...
            if len(cells) >= 2:
                ...
                if len(cells) == 2:
                    table_col_widths = [total_w * 0.3, total_w * 0.7]
                elif len(cells) == 3:
                    table_col_widths = [total_w * 0.05, total_w * 0.55, total_w * 0.4]
                elif len(cells) == 4:
                    table_col_widths = [total_w * 0.05, total_w * 0.35, total_w * 0.25, total_w * 0.35]
                else:
                    col_w = total_w / len(cells)
                    table_col_widths = [col_w] * len(cells)

                if not in_table:
                    pdf.table_row(cells, bold=True, col_widths=table_col_widths)
                    in_table = True
                else:
                    pdf.table_row(cells, col_widths=table_col_widths)
            continue
