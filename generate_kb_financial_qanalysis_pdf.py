#!/usr/bin/env python3
"""Generate a PDF from the KB Financial Group Quartr-enhanced analysis markdown."""

import re
from fpdf import FPDF

INPUT = "KB Financial Group-qanalysis.md"
OUTPUT = "docs/pdfs/KB Financial Group-qanalysis.pdf"
...
class MarkdownPDF(FPDF):
    ...
    def table_row(self, cells, bold=False):
        ...
        for cell_text in cells:
            clean = self._sanitize(cell_text)
            if len(clean) > 45:
                clean = clean[:42] + "..."
            if bold:
                self.set_fill_color(230, 235, 245)
                self.cell(col_w, h, clean, border=1, fill=True)
            else:
                self.cell(col_w, h, clean, border="LBR")
        self.ln(h)
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "KB Financial Group (A105560) - Quartr-Enhanced Investment Analysis", align="C")
            self.set_text_color(0, 0, 0)
            self.ln(12)
...
def parse_and_render(md_path, output_path):
    ...
    pdf.title_page(
        ...
        [
            ...
            "",
            "Date: 5 April 2026",
            "Market Cap: KRW 52.4T (~USD 34.7B) | KRX: A105560",
            "GICS: Diversified Banks | South Korea",
        ],
    )
