#!/usr/bin/env python3
"""Generate a PDF from the Dino Polska Quartr-enhanced analysis."""

import re
from fpdf import FPDF

INPUT = "Dino Polska-qanalysis.md"
OUTPUT = "docs/pdfs/Dino Polska-qanalysis.pdf"
...
class MarkdownPDF(FPDF):
    ...
    def _sanitize(text):
        ...
        text = text.replace("\u00d7", "x")
        text = text.replace("\u2190", "<-").replace("\u2192", "->")
        text = text.replace("\u0119", "e").replace("\u0105", "a")
        text = text.replace("\u0107", "c").replace("\u015b", "s")
        text = text.replace("\u0142", "l").replace("\u017c", "z")
        text = text.replace("\u017a", "z").replace("\u0144", "n")
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "Dino Polska S.A. (DNP) - Quartr-Enhanced Analysis", align="C")
            self.set_text_color(0, 0, 0)
            self.ln(12)
...
def parse_and_render(md_path, output_path):
    ...
    pdf.title_page(
        ...
        [
            ...
            "",
            "Ticker: DNP (Warsaw Stock Exchange)",
            "Market Cap: PLN 32.1B (~USD 8.6B)",
            "Sector: Food Retail | Country: Poland",
            "",
            "Date: 1 April 2026",
        ],
    )
