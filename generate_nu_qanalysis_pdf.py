#!/usr/bin/env python3
"""Generate a PDF from the Nu Holdings Quartr-enhanced analysis markdown."""

import re
from fpdf import FPDF

INPUT = "Nu Holdings-qanalysis.md"
OUTPUT = "docs/pdfs/Nu Holdings-qanalysis.pdf"
...
class MarkdownPDF(FPDF):
    ...
    def _sanitize(text):
        ...
        text = text.replace("\u00d7", "x")
        text = text.replace("\u2190", "<-").replace("\u2192", "->")
        text = text.replace("\u00e9", "e").replace("\u00e1", "a")
        text = text.replace("\u00e3", "a").replace("\u00ed", "i")
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "Nu Holdings (NU) - Quartr-Enhanced Investment Analysis", align="C")
            self.set_text_color(0, 0, 0)
            self.ln(12)
...
def parse_and_render(md_path, output_path):
    ...
    pdf.title_page(
        ...
        [
            ...
            "",
            "Date: 31 March 2026",
            "Market Cap: ~$66 billion | Ticker: NU (NYSE)",
            "GICS: Diversified Banks",
        ],
    )
    ...
    while i < len(lines):
        ...
        if line.startswith("## "):
            pdf.h2(pdf._sanitize(line[3:].strip()))
            continue
        if line.startswith("### "):
            pdf.h3(pdf._sanitize(line[4:].strip()))
            continue
