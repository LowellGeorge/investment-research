#!/usr/bin/env python3
"""Generate a PDF from the ResMed Quartr-enhanced analysis markdown."""

import re
from fpdf import FPDF

INPUT = "Resmed-qanalysis.md"
OUTPUT = "docs/pdfs/Resmed-qanalysis.pdf"
...
class MarkdownPDF(FPDF):
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 10, "ResMed (RMD) - Quartr-Enhanced Investment Analysis", align="C")
            self.set_text_color(0, 0, 0)
            self.ln(12)
...
def parse_and_render(md_path, output_path):
    ...
    pdf.title_page(
        ...
        [
            ...
            "",
            "Date: 5 April 2026",
            "Market Cap: ~$32B | Ticker: RMD (NYSE / ASX)",
            "FY25 Revenue: $5.15B | Net Income: $1.40B | FCF: $1.66B",
        ],
    )
    ...
    # Skip title and metadata lines until first ##
    while i < len(lines):
        line = lines[i].rstrip()
        if line.startswith("## "):
            break
